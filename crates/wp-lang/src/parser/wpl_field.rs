...
fn wpl_id_field<'a>(input: &mut &'a str) -> ModalResult<(&'a str, WplField)> {
    ...
    let mut conf = WplField {
        name: f_key,
        meta_name: meta_type.static_name().to_string(),
        meta_type,
        fmt_conf,
        separator: sep,
        content,
...
fn wpl_field_impl(input: &mut &str) -> ModalResult<WplField> {
    ...
    conf.meta_name = main_meta.clone().to_string();
    ...
    if peek_str(":", input).is_ok() {
        ...
        let f_name = take_var_name
            ...
            .parse_next(input)?;
        conf.name = Some(f_name.to_string());
    }
    if let Some(max_len) =
...
fn parse_peek_symbol(input: &mut &str, conf: &mut WplField) -> ModalResult<()> {
    if conf.meta_type == DataType::PeekSymbol {
        ...
        let content = opt(take_parentheses).parse_next(input)?;
        conf.content = content.map(|x| x.to_string());
        conf.meta_name = DataType::Symbol.to_string();
    }
    Ok(())
}
...
mod tests {
    ...
    fn test_named_field() {
        ...
        assert_eq!(conf.name, Some("src-ip".to_string()));
