...
pub fn bench_nginx_10k(c: &mut Criterion) {
    ...
    let wpl_full_clf =
        r#"(ip:sip,_^2,time/clf<[,]>,http/request\",http/status,digit,chars\",http/agent\",_\")"#;
    // 去掉 time，粗略估算 chrono 路径的占比（两者差值）
    let wpl_no_time =
        r#"(ip:sip,_^2,chars,http/request\",http/status,digit,chars\",http/agent\",_\")"#;
    // 时间换成 timestamp 快路径，粗测“fast path”潜力
    let wpl_epoch =
        r#"(ip:sip,_^2,time/timestamp,http/request\",http/status,digit,chars\",http/agent\",_\")"#;
